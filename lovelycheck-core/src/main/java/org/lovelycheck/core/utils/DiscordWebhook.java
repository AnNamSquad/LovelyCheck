...
public class DiscordWebhook {
    ...
    public static void send(String webhookUrl, String content, String embedTitle, String embedDescription, int embedColor, String embedFooter) {
        ...
        if (!tryAcquireRateLimit()) {
            System.err.println("[lovelycheck] Discord webhook rate limit reached, skipping message");
            return;
        }
        ...
        try {
            ...
            connection.setRequestProperty("User-Agent", "lovelycheck-webhook");
            ...
            if (responseCode >= 400) {
                System.err.println("[lovelycheck] Webhook request failed with status: " + responseCode);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
