...
public class PacketEventsPayloadListener extends PacketListenerAbstract {
    ...
    public PacketEventsPayloadListener(LovelyCheckConnectionPlugin plugin) {
    ...
    private void handlePacket(PacketReceiveEvent event) {
        ...
        if (ForgeConfig.isEnabled()) {
            processForgePacket(playerUuid, playerName, playerData, channel, message);
        }
    }

    private void processForgePacket(UUID playerUuid, String playerName, LovelyCheckPlayer playerData, String channel, String message) {
        ...
        if (ForgeChannelParser.BRAND_CHANNEL.equalsIgnoreCase(channel)) {
            ...
            checkBrandSpoofing(playerUuid, playerName, playerData);
        }
    ...
    private void checkBrandSpoofing(UUID playerUuid, String playerName, LovelyCheckPlayer playerData) {
    ...
    private void performActions(Action action, UUID playerUuid, String playerName, String checkName, TagResolver.Single... templates) {
        ...
        Player player = Bukkit.getPlayer(playerUuid);
        if (player != null && hasBypassPermission(player)) {
            return;
        }
        ...
        if (!isPlayerFullyOnline(playerUuid)) {
            playerData.queuePendingAction(() -> executeCommands(action, playerUuid, checkName));
            return;
        }
