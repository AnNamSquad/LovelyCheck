package org.lovelycheck.spigot.checks.listeners;

import org.lovelycheck.spigot.LovelyCheckPlugin;
import org.lovelycheck.spigot.checks.HackDefinition;
import org.lovelycheck.spigot.checks.utils.MessageUtil;
...
public class AntiCheatListener implements Listener {
    ...
    public AntiCheatListener(LovelyCheckPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void registerHooks() {
        ...
        if (plugin.getConfigManager().isGrimEnabled()) {
            ...
            if (!ok) plugin.getLogger().info("[lovelycheck] Grim not found, skipping.");
        }
        if (plugin.getConfigManager().isVulcanEnabled())
            if (!tryRegister(VULCAN_CLASS, "Vulcan", false))
                plugin.getLogger().info("[lovelycheck] Vulcan not found, skipping.");
        if (plugin.getConfigManager().isSpartanEnabled())
            if (!tryRegister(SPARTAN_CLASS, "Spartan", false))
                plugin.getLogger().info("[lovelycheck] Spartan not found, skipping.");
    }
    ...
    private boolean tryRegister(String className, String acName, boolean isGrim) {
        try {
            ...
            plugin.getLogger().info("[lovelycheck] " + acName + " hook registered (" + className + ").");
            return true;
        } catch (ClassNotFoundException e) {
            return false;
        } catch (Exception e) {
            plugin.getLogger().warning("[lovelycheck] " + acName + " hook failed: " + e.getMessage());
            return false;
        }
    }
    ...
    private void handleFlag(Event event, boolean isGrim, String acName) {
        ...
        plugin.getLogger().info("[lovelycheck] " + acName + " flagged " + player.getName() + " — queuing check.");
        MessageUtil.broadcastAlerts(plugin,
                MessageUtil.parse(plugin, "anticheat-trigger", Map.of("player", player.getName())));

        List<HackDefinition> hacks = plugin.getConfigManager().getFlagLovelyCheck();
    ...
    private Player extractPlayer(Event event, boolean isGrim, String acName) {
        ...
        plugin.getLogger().warning("[lovelycheck] Could not extract player from event: "
