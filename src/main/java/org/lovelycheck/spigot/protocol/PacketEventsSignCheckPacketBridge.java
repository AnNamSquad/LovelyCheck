...
public final class PacketEventsSignCheckPacketBridge implements SignCheckPacketBridge {
    ...
    @Override
    public boolean openFakeSign(Player player, Location location, List<HackDefinition> batch, boolean includeControlLine) {
    ...
    private NBTCompound buildSignNbt(Location location, List<HackDefinition> batch, boolean includeControlLine) {
        ...
        root.setTag("front_text", buildTextSide(batch, includeControlLine));
    ...
    private NBTCompound buildTextSide(List<HackDefinition> batch, boolean includeControlLine) {
        ...
        int maxHackLines = includeControlLine ? 3 : 4;

        for (int i = 0; i < 4; i++) {
            String json = i < maxHackLines && i < batch.size()
